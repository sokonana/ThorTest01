...
void frame_available_callback(void* sender, unsigned short* image_buffer, int frame_count, unsigned char* metadata, int metadata_size_in_bytes, void* context)
{
    ...
    while (meta_idx < metadata_size_in_bytes)
    {
        unsigned char* metadata_item = metadata + meta_idx;
        unsigned long value = *(unsigned long*)(metadata_item + 4);

        if (!strncmp(metadata_item, "ENDT", 4))
        {
            break;
        }
        else
